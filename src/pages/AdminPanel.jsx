...
import axios from 'axios'; // 🔥 Standard Axios import for uplink
...
const AdminPanel = ({ onBack }) => { 
  ...
  const [rawFile, setRawFile] = useState(null); // 🔥 NEW: Capture physical file for backend
  ...
  const handleExcelSync = async () => {
    if (!rawFile) return alert("❌ ERROR: NO_FILE_CAPTURED");
    ...
    try {
      ...
      if (response.data.success) {
        alert(`✨ [SYSTEM]: ARCHIVE_SYNC_COMPLETE.`);
        setStagedData(null); 
        setRawFile(null);
        fetchData(); 
      } else {
        alert("❌ [SYSTEM]: SYNC_FAILED -> " + (response.data.msg || "UNKNOWN_ERROR"));
      }
    } catch (error) {
